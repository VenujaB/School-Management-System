...
<body>
    ...
    <nav class="navbar navbar-expand-lg navbar-dark bg-dark">
        <a class="navbar-brand" href="Home.jsp"> <img src="images/logo.jpg"  width="250px" height="65px" ></a>
        ...
        <div class="collapse navbar-collapse " id="navbarSupportedContent">
          <ul class="navbar-nav mr-auto">
            <li class="nav-item ">
              <a class="nav-link h6 ml-4" href="Home.jsp">Home <span class="sr-only"></span></a>
            </li>
...
<c:forEach var ="view" items = "${BookingDetails}">
	
	<c:set var = "id" value = "${view.getId()}"/>
	<c:set var = "ResourceID" value = "${view.getResourceID()}"/>
	<c:set var = "StuName" value = "${view.getStuName()}"/>
	<c:set var = "StuEmail" value = "${view.getStuEmail()}"/>
	<c:set var = "StuMobile" value = "${view.getStuMobile()}"/>
	<c:set var = "ResourceType" value = "${view.getResourceType()}"/>
	<c:set var = "BookDate" value = "${view.getBookDate()}"/>
	<c:set var = "BookTime" value = "${view.getBookTime()}"/>



	<tr>
	<td>${view.getId()}</td>
	<td>${view.getResourceID()}</td>
	<td>${view.getStuName()}</td>
	<td>${view.getStuEmail()}</td>
	<td>${view.getStuMobile()}</td>
	<td>${view.getResourceType()}</td>
	<td>${view.getBookDate()}</td>
	<td>${view.getBookTime()}</td>
	
	<td><c:url value="StudentBookResourceUpdate.jsp" var="BookingUpdate">
		
		<c:param name="ResourceID" value="${ResourceID}"/>
		<c:param name="StuName" value="${StuName}"/>
		<c:param name="StuEmail" value="${StuEmail}"/>
		<c:param name="StuMobile" value="${StuMobile}"/>
		<c:param name="ResourceType" value="${ResourceType}"/>
		<c:param name="BookDate" value="${BookDate}"/>
		<c:param name="BookTime" value="${BookTime}"/>
...
<input type="Button" class="btn btn-warning" name="update" value="Update">
	...
	<td><c:url value="StudentBookResourceDelete.jsp" var="BookingDelete">
